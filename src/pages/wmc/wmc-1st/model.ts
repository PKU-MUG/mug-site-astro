...
export const parseCSV = (csv: string): Submission[] => {
    ...
    for (const line of lines) {
        ...
        const [id, submitTime, usedTime, source, sourceDetail, ip, gameId, qq, group, screenshot, _videoMethod, videoId, fileName, score] = columns;
        const video = (videoId !== "") ? (["bilibili", videoId] as const) : (["disk", fileName] as const);
        if (score === "") continue;
        submissions.push({
            id: parseInt(id),
            submitTime,
    ...
    for (const [key, group] of grouped.entries()) {
        const maxSubmission = group.reduce((prev, curr) => (prev.score > curr.score ? prev : curr));
